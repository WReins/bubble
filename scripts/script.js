gsap.registerPlugin(MotionPathPlugin, ScrollTrigger)

const rx = window.innerWidth < 1000 ? window.innerWidth / 1200 : 1
const ry = window.innerHeight < 700 ? window.innerHeight / 1200 : 1

const path = [
    { x: 1400, y: 200},
    { x: 800, y: 20},
    { x: 900, y: 100},
    { x: 400, y: 150},
    { x: 600, y: 200},
    { x: 800, y: 20},
]

const scaledPath = path.map(({ x, y }) => {
	return {
		x: x * rx,
		y: y * ry
	}
})

const tl = gsap.timeline({
	scrollTrigger: {
		scrub: 3,
	},
})
tl.to('.bubble', {
	motionPath: {
		path: scaledPath,
		align: 'self',
		alignOrigin: [0.5, 0.5],
	},
	duration: 40,
	immediateRender: true,
    scale: 0.6,
    ease: 'Power2.inOut'
})

let dropBase = createBase({
    element: document.querySelector(".base"),
    numPoints: 10,
    centerX: 600,
    centerY: 600,
    minRadius: 230,
    maxRadius: 240,
    minDuration: 1,
    maxDuration: 3
})

let dropRefraction = createBase({
    element: document.querySelector(".refraction"),
    numPoints: 10,
    centerX: 600,
    centerY: 600,
    minRadius: 130,
    maxRadius: 140,
    minDuration: 1,
    maxDuration: 3,
})

let dropColor = createBase({
    element: document.querySelector(".color"),
    numPoints: 10,
    centerX: 600,
    centerY: 600,
    minRadius: 190,
    maxRadius: 200,
    minDuration: 1,
    maxDuration: 3
})

let dropSpot = createBase({
    element: document.querySelector(".spot"),
    numPoints: 8,
    centerX: 460,
    centerY: 460,
    minRadius: 10,
    maxRadius: 12,
    minDuration: 1,
    maxDuration: 2
})

let dropShine = createHighlight({
    element: document.querySelector(".shine"),
    numPoints: 8,
    centerX: 600,
    centerY: 410,
    minRadius: 48,
    maxRadius: 52,
    minDuration: 1,
    maxDuration: 3
})

let dropShadow = createHighlight({
    element: document.querySelector(".shadow"),
    numPoints: 10,
    centerX: 0,
    centerY: 670,
    minRadius: 68,
    maxRadius: 72,
    minDuration: 1,
    maxDuration: 2
})

function createBase(options) {
    let points = []
    let path = options.element
    let slice = (Math.PI * 2) / options.numPoints
    let startAngle = gsap.utils.random(0, 360)

    let tl = gsap.timeline({
        onUpdate: update
    })

    for (let i = 0; i < options.numPoints; i++) {
        let angle = startAngle + i * slice
        let duration = gsap.utils.random(options.minDuration, options.maxDuration)

        let point = {
            x: options.centerX + Math.cos(angle) * options.minRadius,
            y: options.centerY + Math.sin(angle) * options.minRadius
        }

        let tween = gsap.to(point, {
            duration,
            x: options.centerX + Math.cos(angle) * options.maxRadius,
            y: options.centerY + Math.sin(angle) * options.maxRadius,
            repeat: -1,
            yoyo: true,
            ease: "sine.inOut"
        })

        tl.add(tween, -gsap.utils.random(0, duration))
        points.push(point)
    }

    options.tl = tl;
    options.points = points;
    
    function update() {
      path.setAttribute("d", cardinal(points, true, 1));
    }
    
    return options;
}

function createHighlight(options) {
    let points = []
    let path = options.element
    let slice = (Math.PI * 2) / options.numPoints
    let startAngle = gsap.utils.random(0, 360)

    let tl = gsap.timeline({
        onUpdate: update
    })

    for (let i = 0; i < options.numPoints; i++) {
        let angle = startAngle + i * slice
        let duration = gsap.utils.random(options.minDuration, options.maxDuration)

        let point = {
            x: options.centerX + Math.cos(angle) * options.minRadius,
            y: options.centerY + (Math.sin(angle) * 0.5) * options.minRadius
        }

        let tween = gsap.to(point, {
            duration,
            x: options.centerX + Math.cos(angle) * options.maxRadius,
            y: options.centerY + (Math.sin(angle) * 0.5) * options.maxRadius,
            repeat: -1,
            yoyo: true,
            ease: "sine.inOut"
        })

        tl.add(tween, -gsap.utils.random(0, duration))
        points.push(point)
    }

    options.tl = tl;
    options.points = points;
    
    function update() {
      path.setAttribute("d", cardinal(points, true, 1));
    }
    
    return options;
}

function cardinal(data, closed, tension) {
  
    if (data.length < 1) return "M0 0";
    if (tension == null) tension = 1;
    
    let size = data.length - (closed ? 0 : 1);
    let path = "M" + data[0].x + " " + data[0].y + " C";
    
    for (let i = 0; i < size; i++) {
      
      let p0, p1, p2, p3;
      
      if (closed) {
        p0 = data[(i - 1 + size) % size];
        p1 = data[i];
        p2 = data[(i + 1) % size];
        p3 = data[(i + 2) % size];
        
      } else {
        p0 = i == 0 ? data[0] : data[i - 1];
        p1 = data[i];
        p2 = data[i + 1];
        p3 = i == size - 1 ? p2 : data[i + 2];
      }
          
      let x1 = p1.x + (p2.x - p0.x) / 6 * tension;
      let y1 = p1.y + (p2.y - p0.y) / 6 * tension;
  
      let x2 = p2.x - (p3.x - p1.x) / 6 * tension;
      let y2 = p2.y - (p3.y - p1.y) / 6 * tension;
      
      path += " " + x1 + " " + y1 + " " + x2 + " " + y2 + " " + p2.x + " " + p2.y;
    }
    
    return closed ? path + "z" : path;
  }